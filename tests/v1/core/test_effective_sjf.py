...
from vllm.v1.core.sched.request_queue import EffectiveSJFRequestQueue
...
def test_effective_sjf_queue_orders_recovery_aging_cost_and_ties():
    ...
    queue = EffectiveSJFRequestQueue(
        lambda request: costs[request.request_id], max_wait_ms=1_000, clock=lambda: 100
    )
    ...
    assert queue.peek_request() is short
    assert queue.pop_request() is short
    assert queue.pop_request() is same_cost


def test_effective_sjf_queue_recomputes_cost_but_pins_peeked_request():
...
def test_effective_sjf_queue_supports_requeue_and_removal():
...
def test_effective_sjf_prefers_warm_long_prompt_over_cold_short_prompt():
    scheduler = create_scheduler(
        scheduling_policy="effective_sjf",
    ...
    assert scheduler._get_effective_sjf_cost(cached) == 1
...
def test_effective_sjf_zero_local_hit_uses_prompt_length(enable_prefix_caching: bool):
    scheduler = create_scheduler(
        scheduling_policy="effective_sjf", enable_prefix_caching=enable_prefix_caching
    )
    long_request, short_request = create_requests(
        num_requests=2, num_tokens=20, req_ids=["long", "short"]
    )
    short_request.prompt_token_ids = [1] * 10
    short_request.all_token_ids = short_request.prompt_token_ids.copy()

    assert scheduler._get_effective_sjf_cost(long_request) == 20
    assert scheduler._get_effective_sjf_cost(short_request) == 10
    short_request.skip_reading_prefix_cache = True
    assert scheduler._get_effective_sjf_cost(short_request) == 10


def test_effective_sjf_compares_waiting_and_skipped_waiting_globally():
    scheduler = create_scheduler(scheduling_policy="effective_sjf")
...
def test_effective_sjf_does_not_probe_connector_while_ranking():
    scheduler = create_scheduler(
        scheduling_policy="effective_sjf",
        enable_prefix_caching=True,
...
def test_effective_sjf_config_and_cli_validation():
    config = SchedulerConfig(
        max_model_len=32,
        max_num_batched_tokens=32,
        max_num_seqs=1,
        is_encoder_decoder=False,
        policy="effective_sjf",
    )
    assert config.effective_sjf_max_wait_ms == 60_000

    with pytest.raises(ValueError, match="effective_sjf"):
        SchedulerConfig(
            max_model_len=32,
            max_num_batched_tokens=32,
            max_num_seqs=1,
    ...
    args = parser.parse_args(
        [
            "--scheduling-policy",
            "effective_sjf",
            "--effective-sjf-max-wait-ms",
            "1234",
        ]
    )
    assert args.scheduling_policy == "effective_sjf"
    assert args.effective_sjf_max_wait_ms == 1234
