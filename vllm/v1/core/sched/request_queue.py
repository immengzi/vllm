...
class EffectiveSJFRequestQueue(RequestQueue):
    """A dynamic queue ordered by local prefill work and bounded aging."""

    def __init__(
        self,
        effective_prefill_cost: Callable[[Request], int],
        max_wait_ms: int,
        clock: Callable[[], float] = time.time,
    ) -> None:
        self._requests: list[Request] = []
        self._effective_prefill_cost = effective_prefill_cost
    ...
    def _selection_key(
        ...
        return (
            2,
            self._effective_prefill_cost(request),
...
def create_request_queue(
    ...
    effective_prefill_cost: Callable[[Request], int] | None = None,
    effective_sjf_max_wait_ms: int = 60_000,
) -> RequestQueue:
    ...
    elif policy == SchedulingPolicy.EFFECTIVE_SJF:
        if effective_prefill_cost is None:
            raise ValueError("effective_sjf requires an effective prefill cost")
        return EffectiveSJFRequestQueue(
            effective_prefill_cost, effective_sjf_max_wait_ms
        )
    else:
